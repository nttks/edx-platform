...
class AccessTestCase(LoginEnrollmentTestCase, ModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.course = CourseFactory.create(org='edX', number='toy', run='2012_Fall')
    ...
    def verify_access(self, mock_unit, student_should_have_access):
        ...
        self.assertEqual(
            ...
            access._has_access_descriptor(self.anonymous_user, 'load', mock_unit, course_key=self.course.id)
        )
        self.assertTrue(
    ...
    def test__has_access_error_desc(self):
        ...
        self.assertFalse(access._has_access_error_desc(self.student, 'load', descriptor, self.course.id))
        self.assertTrue(access._has_access_error_desc(self.course_staff, 'load', descriptor, self.course.id))
        self.assertTrue(access._has_access_error_desc(self.course_instructor, 'load', descriptor, self.course.id))

        self.assertFalse(access._has_access_error_desc(self.student, 'staff', descriptor, self.course.id))
        self.assertTrue(access._has_access_error_desc(self.course_staff, 'staff', descriptor, self.course.id))
        self.assertTrue(access._has_access_error_desc(self.course_instructor, 'staff', descriptor, self.course.id))

        self.assertFalse(access._has_access_error_desc(self.student, 'instructor', descriptor, self.course.id))
        self.assertFalse(access._has_access_error_desc(self.course_staff, 'instructor', descriptor, self.course.id))
        self.assertTrue(access._has_access_error_desc(self.course_instructor, 'instructor', descriptor, self.course.id))
    ...
    @patch.dict('django.conf.settings.FEATURES', {'DISABLE_START_DATES': False})
    @patch('courseware.access.get_current_request_hostname', Mock(return_value='preview-lms.localhost'))
