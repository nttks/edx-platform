...
def get_course_enrollment_pairs(user, course_org_filter, org_filter_out_set, sort_order=None):
    """
...
def dashboard(request):
    ...
    course_enrollment_pairs = list(get_course_enrollment_pairs(user, course_org_filter, org_filter_out_set, sort_order=('-created', '-id')))
    ...
    advanced_course_statuses = {
        course.id: AdvancedCourseStatus(request, course)
        for course, _ in course_enrollment_pairs
    }
    ...
    block_courses = frozenset(course.id for course, enrollment in course_enrollment_pairs
                              if is_course_blocked(request, CourseRegistrationCode.objects.filter(course_id=course.id, registrationcoderedemption__redeemed_by=request.user), course.id) or (course.has_terminated() and not staff_access))
...
def password_reset_confirm_wrapper(request, uidb36=None, token=None):
    ...
    try:
        ...
        return password_reset_confirm(
            request, uidb36=uidb36, token=token, extra_context=platform_name
        )
...
def change_email_settings(request):
    ...
    if receive_emails:
        optout_object = Optout.objects.filter(user=user, course_id=course_key, force_disabled=False)
        if optout_object:
            optout_object.delete()
        log.info(
