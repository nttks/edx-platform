...
class CourseOperationSurveyDownload4ManagerTest(InstructorAPISurveyDownloadTestMixin, BizContractTestBase):
    """
    Test instructor survey utf16 for biz endpoint.
    """
    def setUp(self):
        super(CourseOperationSurveyDownload4ManagerTest, self).setUp()
        self.setup_user()
    ...
    def test_get_survey_not_allowed_method(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            response = self.client.get(self.get_url())
        self.assertEqual(405, response.status_code)
    ...
    def test_get_survey(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(CourseOperationSurveyDownload4ManagerTest, self).test_get_survey()

    def test_get_survey_when_data_is_empty(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(CourseOperationSurveyDownload4ManagerTest, self).test_get_survey_when_data_is_empty()

    def test_get_survey_when_data_is_broken(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(CourseOperationSurveyDownload4ManagerTest, self).test_get_survey_when_data_is_broken()


class CourseOperationSurveyDownloadForManagerUTF8Test(CourseOperationSurveyDownloadTest, BizContractTestBase):
    """
    Test instructor survey utf8 for biz endpoint.
    """
...
class LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest(LoginCodeEnabledInstructorAPISurveyDownloadTestMixin, BizContractTestBase):
    """
    Test instructor survey utf16 for biz endpoint.
    """
    def setUp(self):
        super(LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest, self).setUp()
        self.setup_user()
        self._manager = self._create_manager(
            org=self.gacco_organization,
            user=self.user,
    ...
    def test_get_survey(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest, self).test_get_survey()

    def test_get_survey_when_data_is_empty(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest, self).test_get_survey_when_data_is_empty()

    def test_get_survey_when_data_is_broken(self):
        with self.skip_check_course_selection(current_contract=self.contract, current_organization=self.gacco_organization,
                                              current_course=self.course, current_manager=self._manager):
            super(LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest, self).test_get_survey_when_data_is_broken()


class LoginCodeEnabledCourseOperationSurveyDownloadForManagerUTF8Test(LoginCodeEnabledCourseOperationSurveyDownload4ManagerTest, BizContractTestBase):
    """
    Test instructor survey utf8 for biz endpoint.
    """
