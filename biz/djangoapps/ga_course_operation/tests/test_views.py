...
class CourseOperationSurveyDownloadTestMixin(BizContractTestBase):
    ...
    def validate_cp932(self, content):
        # UTF8 no BOM
    ...
    def get_survey_csv_rows_unicode_from_cp932(self, content):
...
class CourseOperationSurveyDownloadCp932Test(CourseOperationSurveyDownloadTestMixin):

    def get_url(self):
        return reverse('biz:course_operation:survey_download_cp932')

    def validate_bom(self, content):
        return self.validate_cp932(content)

    def get_survey_csv_rows_unicode(self, content):
        return self.get_survey_csv_rows_unicode_from_cp932(content)
...
class CourseOperationSurveyDownloadLoginCodeTest(CourseOperationSurveyDownloadTestMixin):
    ...
    def test_survey_download_director(self):
        ...
        # has contract register
        user1 = self._create_submission_user()
        self.create_contract_register(user1, self.contract_auth)
        submission1 = SurveySubmissionFactory.create(**{
            'course_id': self.course_spoc5.id,
            'unit_id': '11111111111111111111111111111111',
            'user': user1,
            'survey_name': 'survey #1',
            'survey_answer': '{"Q1": "1", "Q2": ["1", "2"], "Q3": "submission #1"}',
        ...
        # has not contract register
        user2 = self._create_submission_user()
        CourseEnrollment.enroll(user2, self.course_spoc5.id)
        self.submission2 = SurveySubmissionFactory.create(**{
            'course_id': self.course_spoc5.id,
            'unit_id': '11111111111111111111111111111111',
            'user': user2,
        ...
        with self.skip_check_course_selection(current_contract=self.contract_auth, current_organization=self.contract_org,
                                              current_course=self.course_spoc5, current_manager=_manager):
            response = self.client.post(self.get_url(), {})
        ...
        self.assertEqual(rows[0],
                         u"'Unit ID'\t'Survey Name'\t'Created'\t'User Name'\t'Full Name'\t'Email'\t'Resigned'\t'Unenrolled'\t'Login Code'\t'Q1'\t'Q2'\t'Q3'")
        self.assertEqual(
            rows[1],
            u"'11111111111111111111111111111111'\t'survey #1'\t'%s'\t'%s'\t'%s'\t'%s'\t''\t''\t''\t'1'\t'1,2'\t'submission #1'"
...
class CourseOperationSurveyValueError(CourseOperationSurveyDownloadTestMixin):
    def get_url(self):
        return reverse('biz:course_operation:survey_download')
    ...
    def test_survey_download_director(self):
        ...
        submission1 = SurveySubmissionFactory.create(**{
            ...
            'user': user1,
            'survey_name': 'survey #1',
            'survey_answer': '{"akjgalkjgalkbal;kjf"}',
        })
        # has not contract register
        user2 = self._create_submission_user()
        CourseEnrollment.enroll(user2, self.course_spoc5.id)
        ...
        self.assertEqual(2, len(rows))
        self.assertEqual(rows[0],
                         u"'Unit ID'\t'Survey Name'\t'Created'\t'User Name'\t'Full Name'\t'Email'\t'Resigned'\t'Unenrolled'\t'Login Code'")
        self.assertEqual(
            rows[1],
            u"'11111111111111111111111111111111'\t'survey #1'\t'%s'\t'%s'\t'%s'\t'%s'\t''\t''\t''"
            % (format_for_csv(submission1.created), submission1.user.username, submission1.user.profile.name,
               submission1.user.email)
        )
